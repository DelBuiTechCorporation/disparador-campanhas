    ...
    try {
      ...
      for (const contact of contacts) {
        try {
          ...
          await this.sendSubsequentNodes(graph, firstNode.id, contact, validatedPhone, connection, undefined, session.id);
  ...
  async sendSubsequentNodes(graph: any, currentNodeId: string, contact: any, validatedPhone: string, connection: any, sessionToken?: string, sessionId?: string) {
    try {
      ...
      while (true) {
        ...
        // Processar nós de integração (Chatwoot) antes de continuar
        if (nodeType === 'integration_chatwoot') {
